...
用户终止当前 Run 时，Router 会先停止其已登记的 Run-local Demo 服务并验证 PID 归属，再删除当前
Run 中除 `state.json`、`events.ndjson` 和 `cancellation-receipt.json` 以外的所有内容；回执只保存
回收范围与服务停止结果。该操作不遍历其它 Run，不会停掉项目正式服务，也不回滚已落地的正式代码。
