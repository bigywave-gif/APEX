...
- 将 Gate 1 方案登记与 Runtime Demo 登记改为可恢复事务：先持久化含项目、Run、Session、工件 SHA-256 与受控操作回执的登记收据，再提交状态机变更。
- `status` / `resume` 仅在收据与当前工件、来源操作、项目、Run、Session 全部一致时重放未完成的登记提交；不会跨 Session 复用旧收据，也不会因进程中断重跑视觉生成或要求重复确认。
- 恢复 Runtime Demo 登记后会继续应用已持久化的“直接代码”路线决定，避免已生成 Demo 后再次提示视觉确认或丢失路线选择。
