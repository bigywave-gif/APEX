...
- 角色超时、输入失效、输出无效时必须留下受控操作失败回执；已选角色不得用 `unverified` advisory 替代，恢复后从同一受控步骤继续，不增加确认也不产生假完成。
...
- 已选角色失败时记录 operation receipt 并在当前 Gate 阻断；只有未被当前阶段选择的条件角色才是“不适用”，而不是降级成功。恢复只能在输入、环境证据或状态实际改变后进行。
