...
for (const screen of spec.screens) {
  ...
  const open = call(session, artifactDir, ['open', url], runDir); const resize = open.ok ? call(session, artifactDir, ['resize', String(screen.width), String(screen.height)], runDir) : { ok: false, output: 'open failed' };
...
const result = { status: captured.every(item => item.status === 'captured') && motionSamples.every(item => item.status === 'captured') ? 'passed' : 'failed', kind: evidenceKind, sourceSelectionIds: requiredSourceSelectionIds, evidence: captured, motionSamples, capturedAt: new Date().toISOString(), note: 'Captured screenshots and motion samples are evidence only. Visual approval must be recorded against the frozen Stitch/design baseline.' };
