...
if (state.track === 'existing') {
  ...
  const skeletonRef = state.artifacts?.pageSkeleton;
  const skeletonFile = skeletonRef && path.resolve(runDir, skeletonRef);
  if (!skeletonFile || !skeletonFile.startsWith(`${runDir}${path.sep}`) || !fs.existsSync(skeletonFile)) die('Existing visual planning requires a frozen page-skeleton.json');
  ...
  if (!plan.existingPageSkeleton || plan.existingPageSkeleton.sourceTreeHash !== skeleton.sourceTreeHash || plan.existingPageSkeleton.skeletonHash !== skeleton.skeletonHash || mappingByVisualNode.size !== selectedVisualNodes.size || [...selectedVisualNodes].some(node => !mappingByVisualNode.has(node)) || [...mappingByVisualNode.entries()].some(([node, nodeId]) => !selectedVisualNodes.has(node) || !knownNodeIds.has(nodeId))) die('Existing visual plan must bind every selected visual node exactly once to a current frozen page-skeleton node');
}
...
const requiredBindingKinds = ['layout', 'component', 'style', 'font', 'content'];
if (!requiredBindingKinds.every(kind => selectionKinds.has(kind))) die('source selections must explicitly lock layout, component, style, font, and content choices before the effect image is generated');
for (const [visualNode, kinds] of selectionsByVisualNode) {
  const missing = requiredBindingKinds.filter(kind => !kinds.has(kind));
  if (missing.length) die(`visual node ${visualNode} lacks confirmed ${missing.join(', ')} source selections`);
}
