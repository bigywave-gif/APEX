...
const trackedRoots = ['core/runtime/schemas', 'scripts', 'registry/capability-registry.json', 'manifest.yaml'];
...
function synchronizeDocumentVersions(version) { for (const relative of ['system-docs/product-document.md', 'system-docs/technical-architecture.md', 'system-docs/deployment-and-integration.md']) { const file = path.join(root, relative); fs.writeFileSync(file, fs.readFileSync(file, 'utf8').replace(/- 当前版本：[^\n]+/, `- 当前版本：${version}`)); } }
function apply() {
