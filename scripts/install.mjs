...
const refreshHook = spawnSync(process.execPath, [path.join(installedRoot, 'scripts', 'install-codex-session-refresh-hook.mjs')], { encoding: 'utf8', env: { ...process.env, CODEX_HOME: codexHome } });
if (refreshHook.status !== 0) fail(`Session refresh hook installation failed: ${(refreshHook.stderr || refreshHook.stdout).trim()}`);
...
console.log(JSON.stringify({ installed: true, apexRoot: installedRoot, bridge: globalBridge, stopHook: (stopHook.stdout || '').trim(), refreshHook: (refreshHook.stdout || '').trim(), preflight: preflight.status === 0 ? 'ready' : 'blocked', detail: (preflight.stdout || preflight.stderr).trim() }, null, 2));
