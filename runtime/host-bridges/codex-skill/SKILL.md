...
- **宿主硬性终止保护**：安装时必须执行 `scripts/install-codex-stop-hook.mjs`。该 Codex `Stop` Hook 会读取当前 session 绑定的 run 并复核 Router；只要没有失败动作回执且 `terminalResponseContract.allowed === false`，它就必须阻止本回合结束并注入“继续执行当前受权链”的续跑提示。相同 `currentStep` 再次出现只说明尚未产生完成回执，绝不能因去重放行；该约束同样覆盖 Gate 1、视觉方案、Demo、Stitch、实施冻结及 Gate 2/3 验证的全部自动节点。不得禁用、绕过或用文件卡片代替该保护。已有 `blockingOperation` 时，宿主必须输出一次包含回执、观察错误、失败类别和下一项实际修复条件的阻断报告；不得重试、不得再次注入续跑提示、不得显示“继续”。此机制只对精确绑定的 APEX session 生效：创建或恢复 Run 优先传入真实 Codex thread ID；使用别名时必须由 PreTool bridge 精确登记，不能通过扫描其它 session 或宽松匹配猜测绑定。在 Router 返回精确确认、Demo 路线选择、交付或已执行操作的阻断回执时才自动放行。
