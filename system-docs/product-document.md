...
APEX 命中前端、视觉或交互任务时，先停留在**调用确认**而非自动准入：用户会看到 APEX 会增加的 Gate、来源锁定和运行时验证，并决定是否启用。只有明确的“确认调用 APEX / 使用 APEX”才允许创建项目 Run；改造请求、模型计划或“将使用 APEX”都不能替代用户授权。因此用户不会在尚未选择 APEX 时被带入基线采集、方案或确认门。
